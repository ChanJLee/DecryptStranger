...
import com.theOldMen.Activity.BaseActivity;
...
public class XXService extends BaseService implements XXBroadcastReceiver.EventHandler,
    ...
    private static final String RECONNECT_ALARM = "com.theOldMen.RECONNECT_ALARM";
    ...
    public void saveCard(final VCard vCard){
        if (NetUtil.getNetworkState(this) == NetUtil.NETWORN_NONE) {
            connectionFailed(NETWORK_ERROR);
            return;
        }
    ...
    public ArrayList<String> getRosterEnteries(){
        return mSmackable.getRosterEnteries();
    }
    ...
    // 发送文件
    public void sendFile(final String account, final File filePath) {
//        if (mSmackable != null) {
//            new Thread(new Runnable() {
//                @Override
//                public void run() {
                    mSmackable.sendFile(account, filePath);

//                }
//            }).run();
//        }
//        else
//            L.e("FileTransfer error!");
    }

    // 新增联系人
    public boolean addRosterItem(String user, String alias, String group) {
        try {
            return mSmackable.addRosterItem(user, alias, group);
        } catch (MyException e) {
    ...
    private void connectionSuccessed() {
        ...
        mMainHandler.post(new Runnable() {
            public void run() {
                ...
                if (!isAppOnForeground())
                    notifyClient(from, MyUtil.splitJidAndServer(from), Message.analyseMsgBody(message).msg,
    ...
    public void updateServiceNotification(String message) {
        ...
        String title = PreferenceUtils.getPrefString(this,
                PreferenceConstants.ACCOUNT, "");
        Notification n = new Notification(R.drawable.logo_notify,
                title, System.currentTimeMillis());
        n.flags = Notification.FLAG_ONGOING_EVENT | Notification.FLAG_NO_CLEAR;

        Intent notificationIntent = new Intent(this, MainActivity.class);//联系人主界面
